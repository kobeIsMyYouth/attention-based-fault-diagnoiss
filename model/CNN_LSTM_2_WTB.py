...
class MixA_Module(nn.Module):
    ...
    def forward(self, x, ct):
        ...
        B_WH_C_Fusion = (B_WH_C_AVG + B_WH_C_MAX) / 2
        Distance = 1 - torch.abs(ct - B_WH_C_Fusion)
        Attention_weight = self.softmax(Distance.view(m_batchsize, -1))
...
class CNN_LSTM(nn.Module):
    def __init__(self, seq):
        ...
        self.Conv1 = nn.Sequential(
            nn.Conv2d(1, 5, [3, 3], stride=1, padding=1),
            nn.BatchNorm2d(5),
        ...
        self.Conv3 = nn.Sequential(
            nn.Conv2d(10, 15, [3, 3], stride=1, padding=1),
            nn.BatchNorm2d(15),
            nn.ReLU(inplace=True),
        ...
        self.Conv4 = nn.Sequential(
            nn.Conv2d(15, 15, [3, 3], stride=1, padding=1),
            nn.BatchNorm2d(15),
            nn.ReLU(inplace=True),
    ...
    def forward(self, x, ct):  # x[batch, 1, 52, 52]
        ...
        x2 = self.Conv2(x1_atten)  #  x2[batch, 5, 52, 52]
        ...
        return x1_atten, atten1, x2_atten, atten2, x1, x2, x3, x4, x
