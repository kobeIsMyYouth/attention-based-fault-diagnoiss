...
import os
import dataLoader
from model import CNN_LSTM_1_WTB
import torch
...
parser.add_argument("--dr", default="./data/WTB_icing", type=str, help="数据所在的根路径")
...
X_train, y_train, mask_train = dataLoader.window_process2(origTrainRoot, trainRoot, cfg.dl, cfg.ts, cfg.lb)
...
X_test, y_test, mask_test = dataLoader.window_process2(origTestRoot, testRoot, cfg.dl, cfg.ts, cfg.lb)
...
net = CNN_LSTM_1_WTB.CNN_LSTM(cfg.ts)
...
def test(epoch):
    ...
    if test_acc > best_test_acc:
        ...
        torch.save(state, os.path.join('./checkpoints/WTB_icing/res_26_1_5', cfg.model + '-' + str(epoch) + '.t7'))
        best_test_acc = test_acc
        best_test_acc_epoch = epoch
        print('更新完成')
...
if __name__ == "__main__":
    ...
    np.savetxt("./results/WTB_icing/res_26_1_5/trainlosss.txt", trainlosss)
    np.savetxt("./results/WTB_icing/res_26_1_5/train_accs.txt", train_accs)
    np.savetxt("./results/WTB_icing/res_26_1_5/testlosss.txt", testlosss)
    np.savetxt("./results/WTB_icing/res_26_1_5/test_accs.txt", test_accs)
