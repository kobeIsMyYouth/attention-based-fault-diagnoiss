...
def LabelGen(batch):
    ...
    with open('./%s/%s_failureInfo.csv' %(batch, batch), 'r') as failureInfo:
        ...
        for failure_row in failure:
            failureTime.append([datetime.strptime(failure_row[0], '%m/%d/%Y %H:%M:%S'),
                datetime.strptime(failure_row[1], '%m/%d/%Y %H:%M:%S')])
    ...
    with open('./%s/%s_normalInfo.csv' %(batch, batch), 'r') as normalInfo:
        ...
        for normal_row in normal:
            normalTime.append([datetime.strptime(normal_row[0], '%m/%d/%Y %H:%M:%S'),
                datetime.strptime(normal_row[1], '%m/%d/%Y %H:%M:%S')])
